...
class TeacherLog extends Frame implements ActionListener, WindowListener{
	private SmsPage1 p1;
	TextField tf1;
	private TextField tf2;
	public AdminLog(SmsPage1 f){
		super("Student Management System, Admin Log In");
		p1=f;
		Label l1=new Label("Admin ID");
		...
		this.addWindowListener(new WindowAdapter(){
			publc void windowClosing(WindowEvent e)
			{
				System.exit(0);
			}
		});
	...
	public void actionPerformed(ActionEvent e){
		...
		if(s.equals("Login"))
		{
			...
			String q="select aId, aPassword from admin";
			ResultSet rs=null;	
			try{
				rs=da.getData(q);
				while(rs.next()){
					String un = rs.getString("aId");
					String tp= rs.getString("aPassword");
					if(un.equals(typedName) && tp.equals(typedPass))
					{
